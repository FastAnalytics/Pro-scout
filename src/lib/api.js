...
export const DEVEX = {
  ...
  minimumRobux: 30000,
  minimumUsd: 105,
  note: "Roblox DevEx: 100,000 Robux = $350 USD.",
};
...
function normalizeGame(game) {
  const thumbnailId =
    typeof game.thumbnail === "number" ||
    /^\d+$/.test(String(game.thumbnail || ""))
      ? String(game.thumbnail)
      : "";
  ...
  const icon =
    ...
    (typeof game.thumbnail === "string" && game.thumbnail.startsWith("http")
      ? game.thumbnail
      : thumbnailId
      ? `https://tr.rbxcdn.com/${thumbnailId}/420/420/Image/Png`
      : "");

  return {
    ...game,
    icon,
    thumbnail: game.thumbnail || icon,
    likes: Number(game.likes) || 0,
    dislikes: Number(game.dislikes) || 0,
    ratio:
      game.ratio ??
      (game.visits ? (game.playing || 0) / game.visits : 0),
  };
}
...
async function enrichRobloxGames(games) {
  ...
  for (let offset = 0; offset < enriched.length; offset += 50) {
    ...
    try {
      ...
      const byId = new Map(
        (details.games?.data || []).map((game) => [String(game.id), game]),
      );

      const imageById = new Map(
        (details.thumbnails?.data || []).map((image) => [
          String(image.targetId),
          image.imageUrl,
        ]),
      );
      ...
      batch.forEach((game) => {
        ...
        if (detail && Number(detail.id) > 0) {
          Object.assign(game, {
            ...
            playing:
              Number(detail.playing ?? game.playing) ||
              game.playing ||
              0,
            visits:
              Number(detail.visits ?? game.visits) ||
              game.visits ||
              0,
            favorites:
              Number(detail.favoritedCount ?? game.favorites) ||
              game.favorites ||
              0,
        ...
        game.ratio = game.visits
          ? (game.playing || 0) / game.visits
          : 0;
      });
    } catch (error) {
  ...
  return entries
    .map(([universeId, value]) => {
      ...
      return normalizeGame({
        universeId,
        name: name || `Roblox game ${universeId}`,
        rootPlaceId,
        thumbnail,
  ...
  for (const url of urls) {
    try {
      ...
      if (!response.ok || !contentType.includes("json")) {
        throw new Error(`Rolimon's returned ${response.status}`);
      }
...
async function loadGames() {
  ...
  async function loadStatic() {
    try {
      const games = await loadJson(DATA_URL);
  ...
  async function loadRolimonsGames() {
    try {
      const remote = await loadRemoteGames();
      return remote.map(normalizeGame);
    } catch {
      return [];
    }
  }
  ...
  gamesPromise = (async () => {
    const [staticGames, rolimonsGames, supabaseGames] = await Promise.all([
      loadStatic(),
      loadRolimonsGames(),
      loadSupabaseGames(),
    ]);

    const merged = [...staticGames, ...rolimonsGames, ...supabaseGames];

    const deduped = new Map();
    merged.forEach((g) => {
      deduped.set(String(g.universeId), {
        ...g,
        _haystack: `${g.name || ""} ${g.description || ""}`.toLowerCase(),
      });
    });

    const allGames = [...deduped.values()];
    return enrichRobloxGames(allGames);
  })();
...
async function loadHistory() {
  ...
  historyPromise =
    SUPABASE_URL && SUPABASE_KEY
      ? loadSupabaseRows(
          "game_snapshots",
          "universe_id,playing,captured_at",
          "captured_at.asc",
        )
          .then((rows) =>
            rows.reduce((acc, row) => {
              (acc[row.universe_id] ||= []).push([
                row.captured_at,
                row.playing,
              ]);
              return acc;
...
const SORTERS = {
  ...
  power: (g) => (g.playing || 0) * (g.ratio || 1),
};

function applyFilters(games, params) {
  const q = String(params.q || "").trim().toLowerCase();

  const minCCU = parseBound(params.minCCU || 100);
  const maxCCU = parseBound(params.maxCCU || 0);

  const minVisits = parseBound(params.minVisits || 900000);
  const maxVisits = parseBound(params.maxVisits || 0);
  ...
  return games.filter((g) => {
    ...
    const ccu = g.playing || 0;
    const visits = g.visits || 0;

    if (minCCU && ccu < minCCU) return false;
    if (maxCCU && ccu > maxCCU) return false;

    if (minVisits && visits < minVisits) return false;
    if (maxVisits && visits > maxVisits) return false;

    if (params.genre && g.genre !== params.genre) return false;

    return !params.hasDiscord || !!g.discord;
  });
}
...
export const api = {
  ...
  stats: async () => {
    ...
    const sum = games.reduce((a, g) => a + (g.playing || 0), 0);

    return {
      totalGames: total,
      highCCU: games.filter((g) => (g.playing || 0) >= 100).length,
      hiddenGems: games.filter(
        (g) =>
          (g.visits || 0) < 5000000 && (g.playing || 0) >= 100,
      ).length,
      avgCCU: total ? Math.round(sum / total) : 0,
      topRatio: games.reduce(
        (a, g) => Math.max(a, g.ratio || 0),
        0,
      ),
    };
  },
  ...
  games: async (params = {}) => {
    ...
    const key = SORTERS[params.sort] || SORTERS.playing;
    const dir = params.order === "asc" ? 1 : -1;

    const matches = applyFilters(games, params).sort(
      (a, b) => (key(a) - key(b)) * dir,
    );
  ...
  game: async (universeId) => {
    ...
    const game = games.find(
      (g) => String(g.universeId) === String(universeId),
    );
  ...
  history: async (universeId, range = "7d") => {
    ...
    const points = (history[String(universeId)] || []).map(
    ...
    const days =
      range === "24h" ? 1 : range === "30d" ? 30 : 7;

    const cutoff =
      new Date(points.at(-1).ts).getTime() -
      days * 86400000;
  ...
  genres: async () => {
    const games = await loadGames();
    return {
      genres: [...new Set(games.map((g) => g.genre).filter(Boolean))].sort(),
    };
  },
